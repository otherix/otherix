...
| `cert <path> exists but key <path> missing` (agent) | Partial bootstrap state - a mid-flight crash or manual file deletion left cert without key (or vice-versa) | Delete the orphaned cert + key + CA material under the agent's cert dir (`/opt/otherix/certs/`), then re-run the agent bootstrap. Identity is derived from the cert CN - no sidecar file to clean up |
