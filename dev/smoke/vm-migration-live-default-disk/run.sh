...
#
# Live VM migration of an image-sized (no --disk-gib) VM - regression smoke for
# the SizeGib=0 default-disk bug. A VM created WITHOUT --disk-gib has a boot disk
# row with SizeGib=0; the live-migration manifest then advertised boot
# size_bytes=0, the live TARGET created a 0-byte destination disk, and the
# blockdev-mirror of the real image-sized (e.g. 3.5 GiB) source hung/failed -
# a 30-minute stall, never reaching cutover. EVERY existing live-migration smoke
# passes --disk-gib 10, so none of them exercised this path. This smoke creates
# the migrant VM-A WITHOUT --disk-gib (image-sized boot disk) and asserts the
# LIVE migration completes and the data plane follows the guest - which can only
# happen if the destination disk was sized to the image and the mirror succeeded.
#
# It is otherwise a faithful copy of vm-migration-live-overlay: an OVERLAY network
# is used (overlay crosses nodes; a plain bridge would not, so connectivity would
# break and mask the disk-size signal), a fresh net name + subnet (10.74.0.0/24)
# that does NOT clash with the overlay smoke's 10.72.0.0/24, and the same
# corrected cloud-init (static netplan IP + belt-and-suspenders ip-addr-replace,
# bash /dev/tcp peer probe, incrementing OVERLAY_PING_OK serial counter).
#
...
#   id).
#
#   create A (NO --disk-gib) + B -> both running, both OVERLAY_PING_OK counters
#                          climbing (cross-node overlay reachability both ways).
#   migrate A (live)    -> node-1 -> node-2; with the bug, the live cutover would
#                          hang here forever (0-byte dest disk + mirror of the
#                          image-sized source). The fix sizes the dest disk to the
#                          image, so the migration record reaches 'completed'
#                          (live=true) and VM-A's current node becomes node-2.
#   THE TEETH           -> within a few seconds of cutover the OVERLAY_PING_OK
#                          counters keep advancing both directions (within the
#                          stall threshold), proving the mirror succeeded and the
#                          data plane followed the guest - no 30-minute hang.
#
...
#
# Usage: make smoke-vm-migration-live-default-disk
#        (or: bash dev/smoke/vm-migration-live-default-disk/run.sh)
...
NET="${NET_NAME:-migdd-smoke}"                # overlay network; delete-firsted (distinct from overlay smoke's migovl-smoke)
VMA="${VMA_NAME:-migdd-a}"                    # the migrant (created WITHOUT --disk-gib)
VMB="${VMB_NAME:-migdd-b}"                    # the stationary peer
SUBNET="10.74.0.0/24"                         # plain overlay subnet (no CP IPAM); distinct from overlay smoke's 10.72.0.0/24
IPA="10.74.0.10"                              # VM-A fixed overlay IP
IPB="10.74.0.11"                              # VM-B fixed overlay IP
...
#     hence bash /dev/tcp (NOT dash sh - dash has no /dev/tcp and always fails).
...
# --- preconditions -----------------------------------------------------
echo "=== vm-migration-live-default-disk smoke: preconditions ==="
...
# --- step 3: VM-A on node-1 (the migrant, NO --disk-gib) ---------------
echo "=== step 3: create $VMA on $NODE1 ($IPA, image-sized boot disk, NO --disk-gib) -> running ==="
# NO --disk-gib here: VM-A's boot disk row gets SizeGib=0 and is image-sized at
# materialisation. This is the whole point of the regression - the live migration
# of THIS VM must still complete (with the bug it hangs: 0-byte dest disk + mirror
...
info "$VMA id=$VMID_A on $NODE1 (image-sized boot disk, no --disk-gib)"
pass "$VMA created and running on $NODE1 (image-sized boot disk)"
...
# No --offline: live is the default. With the SizeGib=0 bug this is where the
# migration hangs (the live target made a 0-byte dest disk and the blockdev-mirror
# of the real image-sized source never completes). The fix sizes the dest disk to
# the image, so the cutover completes within MIGRATE_WAIT.
otx vm migrate "$VMA" --node "$NODE2" \
  --wait --wait-timeout "${MIGRATE_WAIT}s" \
  || fail "vm migrate (live) did not complete within ${MIGRATE_WAIT}s (SizeGib=0 default-disk regression?)"
...
trap - EXIT
echo
echo "${GREEN}=== vm-migration-live-default-disk smoke PASSED (image-sized VM live-migrated, no --disk-gib) ===${NC}"
