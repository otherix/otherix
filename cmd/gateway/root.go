...
const (
	...
	defaultConfigPath = "/etc/otherix/gateway.yaml"
)

// newRootCmd builds the otherix-gateway command tree. The bare binary
// (no subcommand) defaults to `serve` so systemd units
// (ExecStart=/usr/local/bin/otherix-gateway) work without modification.
func newRootCmd() *cobra.Command {
	root := &cobra.Command{
		Use:           "otherix-gateway",
		Short:         "Otherix VM-less ingress gateway",
