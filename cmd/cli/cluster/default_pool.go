...
informational line к stdout и exits 0 (parseable by tooling that
...
func newSetDefaultPoolCommand() *cobra.Command {
	cmd := &cobra.Command{
		...
		Long: `PUT /v1/cluster/default-pool с the supplied pool name. The server
validates that at least one pool instance с this name exists в the
...
stores а logical pool concept, not а specific per-node instance.
Pass а UUID literal here и the server returns 400 pool_not_found.`,
...
func newUnsetDefaultPoolCommand() *cobra.Command {
	cmd := &cobra.Command{
		...
		Long: `DELETE /v1/cluster/default-pool. Idempotent на server (clearing an
already-unset reference returns 204). Requires admin (cluster:manage).
Non-interactive use without --force is а usage error — the operation
mutates cluster-wide state, so the CLI prompts when stdin is а TTY.`,
