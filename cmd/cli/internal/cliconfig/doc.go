...
//
// The on-disk format is a small YAML document with four top-level
// fields (apiVersion / kind / current-cluster / clusters). The
// shape is deliberately a transparent subset of kubectl's
// kubeconfig:
//
//   - apiVersion + kind sentinel — leaves room for a future
...
//     kubectl's contexts/current-context split with the user/auth
//     dimension folded in (we have only bearer tokens, not the
...
//     We borrow the home-relative path and the apiVersion / kind
...
//     path but pick YAML for the hand-edit ergonomics and cluster
//     pointer for multi-environment workflows.
//   - aws: ~/.aws/{config,credentials}, INI, profile-driven with
...
//   - gcloud: ~/.config/gcloud, multi-file directory with named
//     configurations. We deliberately stay with a single file: the
//     credential surface is small enough that a directory's overhead
//     outweighs its flexibility.
//
// # Atomicity and permissions
//
// Save writes a sibling temp file, chmods to 0600, fsyncs, and
// renames onto the target. The renaming is atomic on the local
// filesystem; a crash during Save cannot leave half-written
// credentials. The parent directory is created with 0700 on first
// run. Hand-editors with looser umasks may end up at 0644 — Load
// does not enforce 0600 (would be aggressive), but a future
// `otherix config doctor` could warn.
//
// # Precedence
//
// Endpoint and token are resolved independently and in parallel —
...
// versus current-context: explicit flag is a one-shot override
