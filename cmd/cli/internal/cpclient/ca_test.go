...
func TestGetCA_HappyPath(t *testing.T) {
	...
	if !strings.HasPrefix(out.CertPEM, "-----BEGIN CERTIFICATE-----") {
		t.Errorf("CertPEM missing PEM header: %q", out.CertPEM)
	}
	if out.FingerprintSHA256 != strings.Repeat("a", 64) {
		t.Errorf("FingerprintSHA256 = %q, want 64 'a' chars", out.FingerprintSHA256)
	}
}
