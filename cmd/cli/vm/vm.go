...
// Local flag names — endpoint и token now live on the root cobra
// command (persistent), inherited automatically by everything в
...
// NewCommand returns the `otherix vm` subcommand group, ready К be
// registered onto the root cobra tree by main.
func NewCommand() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "vm",
		Short: "Manage virtual machines (CP /v1/vms surface)",
		Long: `vm groups the operator subcommands против the Control Plane's
/v1/vms surface — create, get, list, delete. Authentication and
endpoint resolution flow through the root-level --endpoint /
--token / --cluster / --config flags (see 'otherix --help' и
'otherix config' для the kubectl-style cluster store).`,
	}
