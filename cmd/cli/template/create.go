...
// для the YAML file/stdin path (flagCloudInitPath). The boolean was
// renamed к keep both useful and unambiguous; tests in
// create_test.go that exercise the boolean were updated в the same
// landing.
const (
...
func newCreateCommand() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "create <name>",
		Short: "Create a template (and optionally materialise its image к a pool).",
...
This command performs up to two operations:
  ...
  2. If --pool is provided, triggers an async image-import task
     ...
     to fetch, verify (or compute), and stage the image. Use --wait к
...
command after а partial failure (e.g. register OK, materialise rejected)
to retry the materialise step.

To register without materialising, omit --pool. To materialise later,
re-run the same command with --pool when ready. To publish а private
...
Two checksum trust modes:
  ...
  - **Verify mode** — pass --expected-sha256. The agent verifies the
    streamed bytes during download и fails fast on mismatch. Use when
    the operator has а trusted upstream checksum (SHA256SUMS file,
    signed release, compliance requirement).
  - **Compute mode** — omit --expected-sha256. The agent computes the
    sha256 during streaming download и returns it; the Control Plane
...
Example (compute mode):
  otherix template create ubuntu-jammy \
	...
	// Optional — when set, runs в verify mode (agent fails fast on
	// checksum mismatch). When absent или empty, runs в compute mode
	...
	cmd.Flags().String(flagImageFormat, "", "image format (qcow2|raw) — server defaults к qcow2")
	cmd.Flags().String(flagFirmwareType, "", "firmware type (bios|uefi) — server defaults к uefi")
	...
	cmd.Flags().String(flagCloudInitSupported, "", "cloud-init supported (true|false) — server defaults к true")
	cmd.Flags().String(flagCloudInitPath, "", "path к а `#cloud-config` YAML file baked into the template; use '-' к read stdin")
	cmd.Flags().String(flagQemuGuestAgent, "", "qemu guest agent expected (true|false) — server defaults к false")

	// Materialisation knobs.
	cmd.Flags().String(flagCreatePool, "", "if set, materialise the image к this pool (name or UUID)")
...
type createFlags struct {
	...
	// content read от the --cloud-init flag (file path or stdin).
	// Nil когда the flag was not set; pointer-к-empty-string allowed
	// и forwarded к the server so the operator can explicitly clear
	// а previously-set pre-bake on PATCH paths (create uses non-empty
	// only — empty file is а warning, not а bake).
...
//
//  1. Always: POST /v1/templates (registration). 409 → fall through к
//     step 2 (idempotent retry).
//  2. If --pool is set: POST /v1/templates/{name}/images
//     (materialisation).
//  3. If --wait is set: poll the materialisation task до terminal.
//
// Partial failure (register OK, materialise rejected) surfaces a
// non-zero exit с а retry hint; re-running the same command resumes
// from the materialise step thanks к 409-fall-through.
func runCreate(cmd *cobra.Command, args []string) error {
	...
	if registered {
		...
		printf(cmd, "template %s already exists; proceeding к materialisation\n", flags.name)
	}
	...
	if err != nil {
		// Registration is durable; materialisation failed. Surface а
		// retry-friendly message — the same command re-run will
		// fall-through registration и re-attempt materialise.
		return fmt.Errorf("materialise: %w (template %s remains registered; re-run the same command к retry)",
...
// registered template plus а boolean indicating whether this call
// performed the insert. On 409 (ErrTemplateExists) it falls through к
// а GET so the caller still gets a fully-populated Template к render.
...
// parseCreateFlags lifts cobra-level flag values into а createFlags
...
// optionalString returns а pointer to the flag's value when the operator
// supplied а non-empty string, nil otherwise. Lets the JSON encoder omit
...
// optionalInt32 returns а pointer to the int32 flag's value when the
// operator supplied а positive number, nil otherwise. Zero is treated
...
// validator. Validation warnings land on cmd's stderr но do not block
// dispatch (locked decision: warn, не gate). Validation errors —
...
// optionalBool reads а string flag and converts true/false к а *bool.
// Empty string means "unset" so server defaults apply. Other strings
// surface as an error so the operator gets а clear validation message
// rather than а silent fall-through.
...
// renderCreateResult prints the registered template в the requested
