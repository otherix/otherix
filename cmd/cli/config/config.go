...
// The subcommands manage a kubectl-style YAML credential store at
// $OTHERIX_CONFIG / ~/.otherix/config — see the cliconfig package
// for the storage layer and precedence semantics.
package config

import "github.com/spf13/cobra"

// NewCommand returns the root `otherix config` command, populated
// with the add / list / use / remove / show children. The tree is
// constructed lazily per call so tests can rebuild a fresh CLI
// without process-global cobra state.
func NewCommand() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "config",
		Short: "Manage CLI cluster credentials (`otherix config add cluster`, list, use, remove, show)",
		Long: `config manages a small YAML credential store on disk so otherix
...
The model mirrors kubectl: each named cluster carries a server URL
plus an API token, and a current-cluster pointer picks which one
unspecified subcommands use. The --cluster flag overrides for one
invocation, and --token / --endpoint / $OTHERIX_API_TOKEN /
