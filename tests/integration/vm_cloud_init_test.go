...
// seed a template with baked YAML, POST /v1/vms with own user_data, assert
// the agent-mock observes the VM-level blob, not the template's.
...
// falls back to template.cloud_init_user_data when vm.user_data is
...
// that when both template AND VM omit user_data, the agent receives an
...
// wins over a template's baked cloud_init_user_data. End-to-end: seed
// a template with baked YAML, create a VM with cloud_init_disabled=true,
// assert the agent-mock observes empty UserData (no cidata.iso would
// be generated by a real agent).
