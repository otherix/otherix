...
func TestWriteBlockingResources_CodeOverride(t *testing.T) {
	rec := httptest.NewRecorder()
	req := httptest.NewRequest(http.MethodDelete, "/v1/templates/abc", nil)

	WriteBlockingResources(rec, req, &BlockingResourcesError{
		Code:      CodeResourceInUse,
		Kind:      "template",
		Message:   "template still has materialised storage images",
		Resources: map[string]int64{"storage_images": 3},
	})
	...
	if got, _ := body.Error.Details["kind"].(string); got != "template" {
		t.Errorf("details.kind = %v, want \"template\"", body.Error.Details["kind"])
	}
}
