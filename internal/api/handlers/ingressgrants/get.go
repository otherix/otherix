...
// Get implements GET /v1/ingress-grants/{id}. Required permission:
// vm:ingress-grant. Ownership is checked against the grant's creator; a
...
// enforces the caller's vm:ingress-grant scope against the grant's creator.
...
// failure.
func (h *Handler) loadOwnedGrant(w http.ResponseWriter, r *http.Request) (store.IngressGrant, bool) {
	caller := auth.UserFromContext(r.Context())
	if caller == nil {
		response.WriteError(w, r, http.StatusUnauthorized,
			response.CodeUnauthenticated, "missing principal", nil)
		return store.IngressGrant{}, false
	}
	...
	grant, err := h.store.IngressGrantByID(r.Context(), id)
	if err != nil {
...
func writeGrantNotFound(w http.ResponseWriter, r *http.Request) {
	response.WriteError(w, r, http.StatusNotFound,
		response.CodeNotFound, "ingress grant not found", nil)
}
