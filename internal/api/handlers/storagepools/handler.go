...
// live в cluster_settings. `is_cluster_default` here is a derived
// boolean: true когда this instance's pool name matches
...
// Nullable on the wire — а pool без
// active pressure has the parent `disk_pressure` field set к JSON
...
// components/schemas/PoolConceptView в api/openapi/control-plane.yaml.
...
// toViewEffective projects а pool_effective_capacity row onto its
// public storagePoolView, substituting the pre-resolved node name. All
// read paths (Get / List / GetByName) и write-path responses (Create /
...
// can spot а divergence (pending-VM-disk pressure) at а glance.
...
// nullable wire field. Returns nil когда the node row was soft-
// deleted out от under the pool (orphaned-pool path в projectView /
...
// toDiskPressureView projects the (since, count) pair on а view-backed
// pool row into the nullable wire shape.
// Returns nil когда the condition is not active — `since == nil`.
...
// into the public storagePoolView. The caller supplies а store.StoragePool
// — typically the one resolver.Pool returned — purely for the ID и
