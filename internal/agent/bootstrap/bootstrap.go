...
// defaultRequestTimeout caps a single HTTP round-trip during bootstrap.
// 30 seconds absorbs a cold TLS handshake plus the CP's redemption
// transaction, with margin for slow networks. Override per-config via
// BootstrapConfig.RequestTimeout when needed.
const defaultRequestTimeout = 30 * time.Second

// nodeCNPrefix is the Subject-CN / SAN-DNS identity prefix the CSR carries.
// The CP signs the leaf with node-<name> per auth.SignCSR regardless of the
// node's role; the CSR prefix is populated for realism and audit.
...
// cfg and returns the issued cert material. Caller is responsible for
// persisting the result via Persist.
//
// Steps (each logged at INFO):
//
//  1. Resolve token plaintext (literal field or TokenPath file).
...
//
// Failure semantics: every step short-circuits with a descriptive
...
// CSR identity prefix (currently always node-).
func bootstrapWithIdentity(ctx context.Context, cfg *config.BootstrapConfig, log *slog.Logger, cnPrefix string) (*Result, error) {
	...
	resp, err := submitCSR(ctx, cfg, token, string(csrPEM), caCert, timeout)
	if err != nil {
	...
	if err := verifyResponseChain(resp.CertPEM, resp.CACertPEM, expectedFingerprint); err != nil {
		log.ErrorContext(ctx, "bootstrap: response chain verification failed — refusing to persist potentially compromised cert material",
			slog.String("token_hash_prefix", hashPrefix),
			slog.String("error", err.Error()))
	...
	log.InfoContext(ctx, "bootstrap: cert issued and chain verified",
...
// resolveToken returns the plaintext join token, reading from
// cfg.TokenPath if cfg.Token is empty. File contents are
// whitespace-trimmed; an empty result after trimming is treated as
// missing.
//
// The token file is NOT deleted after a successful read — operators
// own the cleanup decision (filesystem audit trail benefit or
...
func resolveToken(cfg *config.BootstrapConfig) (string, error) {
	...
	if cfg.TokenPath == "" {
		return "", errors.New("bootstrap: token or token_path is required")
	}
...
// Mirrors the CP-side forensics tag in internal/api/handlers/nodejoin —
