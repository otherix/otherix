...
// bootstrap protocol. On а freshly-installed agent с no cert material
...
// ECDSA P-384 keypair, builds а CSR, и redeems the supplied join
// token at /v1/nodes/join. The returned cert + key + CA + node-id
// are then persisted к disk atomically.
//
// First-request TLS posture is `InsecureSkipVerify: true` - the CP's
// TLS server certificate is не necessarily issued by the cluster CA
...
//     fingerprint.
//  2. The leaf cert returned by /v1/nodes/join must chain back к the
//     same CA whose fingerprint just matched.
//
// Both checks must pass before any file is written к disk. Failures
// short-circuit with descriptive errors — see the FingerprintMismatchError
// и ErrCSRRejected sentinels.
//
...
// и the operator must mint а fresh one to retry.
package bootstrap
