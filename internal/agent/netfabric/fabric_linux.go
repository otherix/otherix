...
type linuxFabric struct {
	...
	// methods (BridgeExists, VXLANExists, WireGuardExists, ListTaps, FDBList,
	// WireGuardPeers) do not lock; none of them calls a mutator, so locking the
	// mutators alone cannot self-deadlock.
	mu sync.Mutex
}
