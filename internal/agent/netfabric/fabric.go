...
type Fabric interface {
	...
	// assigns addr to it, idempotently. It is the unicast counterpart of
	// EnsureAnycastGateway: an ingress gateway claims a distinct per-membership
	// unicast MAC drawn from the network's address space - never the shared
	// anycast MAC, which is identical on every node and can never be a unicast
	// FDB target - so the host kernel originates and answers at the tenant addr
	// and return traffic to the MAC advertised in the overlay FDB is delivered to
	// this bridge. This method owns the bridge hardware address on a gateway,
	// which therefore does not run the anycast services plane.
	EnsureUnicastGateway(bridge string, addr netip.Addr, mac net.HardwareAddr) error
