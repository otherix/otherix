...
func TestNetworksHandleResponseStoresSelfOverlayIP(t *testing.T) {
	rec, err := NewNetworks(&netfabric.FakeFabric{}, nil, discardLogger(), time.Minute, true)
	if err != nil {
...
func TestNewNetworks_RejectsNilFabric(t *testing.T) {
	_, err := NewNetworks(nil, nil, discardLogger(), 0, true)
...
func TestNewNetworks_DefaultsTick(t *testing.T) {
	rec, err := NewNetworks(&netfabric.FakeFabric{}, nil, discardLogger(), 0, true)
	if err != nil {
		t.Fatalf("NewNetworks: %v", err)
	}
...
func TestReconcile_ManagedPlainBridge(t *testing.T) {
	...
	rec, _ := NewNetworks(fab, nil, discardLogger(), DefaultTickInterval, true)
...
func TestRun_NetworksHandlesTriggerAndTick(t *testing.T) {
	fab := &netfabric.FakeFabric{}
	rec, _ := NewNetworks(fab, nil, discardLogger(), 20*time.Millisecond, true)
