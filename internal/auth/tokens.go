...
// API tokens (`otx_*`), refresh tokens, and join tokens
// (`otx_join_*`). All three callers store sha256(plaintext) so
// lookup-by-hash is a constant-time string compare on equal digests
// and no per-token salt is needed (the plaintext already carries 256
