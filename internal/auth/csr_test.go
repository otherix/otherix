...
func TestSignCSRControlAndIngressHostsInSAN(t *testing.T) {
	t.Parallel()
	now := time.Date(2026, 5, 13, 12, 0, 0, 0, time.UTC)
	caResult, err := auth.GenerateClusterCA(now)
	if err != nil {
	...
	pemBytes, _ := generateValidCSR(t, "node-edge1")
	csr, err := auth.ValidateCSR(pemBytes)
	if err != nil {
		t.Fatalf("ValidateCSR: %v", err)
	}

	// A gateway node advertises a control endpoint and an ingress endpoint whose
	// host differs; both hosts must land in the single node- leaf's SAN.
	certPEM, parsed, err := auth.SignCSR(csr, "edge1",
		"https://gw-ctl.example:9443", caCert, signer, now, "https://gw-ingress.example:9444")
	if err != nil {
		t.Fatalf("SignCSR: %v", err)
	}
	...
	if parsed.Subject.CommonName != "node-edge1" {
		t.Errorf("Subject CN = %q, want node-edge1", parsed.Subject.CommonName)
	}
	if !hasDNS(parsed.DNSNames, "node-edge1.agents.otherix.local") {
		t.Errorf("DNSNames = %v, want node-edge1.agents.otherix.local", parsed.DNSNames)
	}
	if !hasDNS(parsed.DNSNames, "gw-ctl.example") {
		t.Errorf("DNSNames = %v, missing control host gw-ctl.example", parsed.DNSNames)
	}
	if !hasDNS(parsed.DNSNames, "gw-ingress.example") {
		t.Errorf("DNSNames = %v, missing ingress host gw-ingress.example", parsed.DNSNames)
	}

	// The unified leaf chains to the cluster CA.
	roots := x509.NewCertPool()
	roots.AddCert(caCert)
	if _, err := parsed.Verify(x509.VerifyOptions{
		...
		t.Errorf("unified leaf chain verification failed: %v", err)
	}
}
