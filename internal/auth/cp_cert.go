...
// CPCertValidity is the lifetime of а freshly generated per-replica
// CP server cert. 1 year matches LeafCertValidity (Step 2 agent
// certs) — cert rotation lifecycle (manual + near-expiry) tracked в
// ROADMAP Planned, deferred к а follow-up iteration.
const CPCertValidity = 365 * 24 * time.Hour

// CPCertNearExpiryBuffer is the window before NotAfter at which а
// cached cert is considered stale enough к regenerate. 30 days lets
// operators schedule replica restarts without surprise TLS handshake
// failures под steady-state.
...
// pattern established elsewhere в the codebase.
var hostnameFn = os.Hostname

// GenerateReplicaCert mints а per-replica CP server cert signed by
// the cluster CA. Template parameters:
//
//   - Subject CN: "otherix-cp-replica" + Organization "Otherix".
//   - Validity: validity from NotBefore (с 1m skew tolerance) к
//     NotBefore+validity.
...
//   - SAN: dnsNames + ips. Both can be empty individually но at least
//     one entry total is required so the cert has а bindable identity.
...
//
// PEM encoding mirrors GenerateClusterCA: CERTIFICATE block для cert,
// PKCS#8 PRIVATE KEY block для key (more portable than SEC1).
func GenerateReplicaCert(caCert *x509.Certificate, caKey crypto.Signer, dnsNames []string, ips []net.IP, validity time.Duration, now time.Time) (certPEM, keyPEM []byte, err error) {
	...
	if len(dnsNames) == 0 && len(ips) == 0 {
		return nil, nil, errors.New("cp_cert: at least one DNS name or IP required для SAN")
	}
	if validity <= 0 {
...
//   - os.Hostname() — added as DNS name когда non-empty.
//   - listenAddr's host portion — added if non-wildcard (parsed
//     via net.SplitHostPort). IP literal goes к ips slice, anything
//     else goes к dnsNames.
//
// Wildcards skipped: "" (port-only), "0.0.0.0", "::", "[::]" (the
// last form arrives при net.SplitHostPort returning unbracketed "::"
// but the constant is included для defense-in-depth).
//
// Operator extends с CPCertConfig.AdditionalSANs at the caller; the
// merge happens в the orchestrator (LoadOrGenerateCPCert) so this
// helper stays focused на the auto-detected baseline только.
...
// от config) into DNS names + IP addresses. Parsing via net.ParseIP
// distinguishes the two — anything that fails IP parsing falls back
// к а DNS name (no further validation; openssl / x509 surface
// malformed entries at cert generation OR handshake time).
//
// Used by LoadOrGenerateCPCert к merge CPCertConfig.AdditionalSANs
...
// MergeUniqueSANs combines auto-detected baseline с operator-supplied
// additions, preserving order и suppressing duplicates. DNS names
...
// assert dedup behavior без going through the orchestrator.
...
// CertCacheValid reports whether the cached cert satisfies все reuse
// invariants required для restart efficiency. Returns nil on valid;
// returns а descriptive error otherwise so the caller's structured
// log carries the rejection reason (operator debugging).
//
// Invariants checked:
//
//  1. NotAfter > now + CPCertNearExpiryBuffer — proactive rotation
//     window. Operators see expiring certs во время restart instead of
//     mid-handshake.
//  2. cert.DNSNames superset of expectedDNS — config may have grown
//     additional_sans since cert generation. Subset failure (cert
//     missing а required name) triggers regenerate. Operator shrinking
//     additional_sans does NOT invalidate (cert covers more than needed
//     stays usable).
//  3. cert.IPAddresses superset of expectedIPs — same logic для IPs.
//  4. Cert chains к currentCA via x509.Verify с the leaf's intended
...
func CertCacheValid(cert *x509.Certificate, expectedDNS []string, expectedIPs []net.IP, currentCA *x509.Certificate, now time.Time) error {
	...
	if _, err := cert.Verify(opts); err != nil {
		return fmt.Errorf("cert chain к current CA invalid: %v", err)
	}
	return nil
}

// WriteCertCacheAtomic writes the cert + key к local paths via the
// tempfile + rename pattern, мirror'я internal/agent/bootstrap.
// writeFileAtomic. Both files land с the requested permissions от
// moment one (Chmod before rename), и а partial write is never
// observable by concurrent readers.
//
// Modes:
//   - cert: 0644 (world-readable, operator-inspectable).
//   - key:  0600 (owner-only, secret).
//   - parent dir (когда created): 0750 (group-readable, world-not).
//
// Cert file written first так что а subsequent boot observing key-
...
// cert is what а chain-check reads first — но neither ordering is
...
func WriteCertCacheAtomic(certPath, keyPath string, certPEM, keyPEM []byte) error {
	if certPath == "" || keyPath == "" {
		return errors.New("cert_path и key_path are required")
	}
	if len(certPEM) == 0 || len(keyPEM) == 0 {
		return errors.New("cert_pem и key_pem must be non-empty")
	}
...
// private к cp_cert так что mode invariants stay co-located с the
// caller. Parent dir is created at 0750 если absent.
