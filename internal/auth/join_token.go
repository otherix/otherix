...
// the wrong slot. The family marker keeps а future single `otx_*`
...
// GenerateJoinToken returns a fresh join-token bundle: the plaintext к
// hand back to the operator exactly once on creation, и the storage
// hash for the join_tokens.token_hash column. There is no display
// prefix — join tokens are short-lived и never surface in any UI
...
// Step 2 redemption handler uses this к fail fast on tokens of the
// wrong subtype (e.g. an API token mistakenly submitted к the
// bootstrap endpoint).
